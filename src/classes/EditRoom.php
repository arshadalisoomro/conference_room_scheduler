...
class EditRoom {
	...
	function getReserEdit($db, $post, $userId, $reserve_id) {
		$tableType = $post['type']; // will either be me, users, or all
		
		$reservation_list_json=array();
...
//reservation
	   $query="SELECT * FROM reservation WHERE reservation._id=".$reserve_id;
        try {
            $stmt = $db->prepare($query);
            $result = $stmt->execute();
			...
			$each_record=array(			
				"id" => $row['_id'],
				"user_id" => $row['user_id'],
				"comference_id" => $row['conference_room_id'],
				"time_slot" => $row['time_slot_id'],
				"recurence_id" => $row['recurrence_id'],
				"date" => $row['date'],
			);
            array_push($reservation_list_json, $each_record);
			...
			echo json_encode($reservation_list_json);
